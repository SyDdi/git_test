...
public class PlateLocate {
    ...
    public Vector<Mat> plateLocate(Mat src) {
        ...
        if (debug) {
            imwrite("d:/tmp/debug_GaussianBlur.jpg", src_blur);
        }

        // Convert it to gray 将图像进行灰度化
        cvtColor(src_blur, src_gray, CV_RGB2GRAY);
        if (debug) {
            imwrite("d:/tmp/debug_gray.jpg", src_gray);
        }
        ...
        if (debug) {
            imwrite("d:/tmp/debug_Sobel.jpg", grad);
        }
        ...
        if (debug) {
            imwrite("d:/tmp/debug_threshold.jpg", img_threshold);
        }
        ...
        if (debug) {
            imwrite("d:/tmp/debug_morphology.jpg", img_threshold);
        }
        ...
        if (debug) {
            ...
            imwrite("d:/tmp/debug_Contours.jpg", result);
        }
        ...
        if (debug) {
            imwrite("d:/tmp/debug_result.jpg", result);
        }
        return resultVec;
    }
     ...
     * @return 打过马赛克后的图片
     */
    public int plateLocate(String imgPath) {
        ...
        for (int i = 0; i < rects.size(); i++) {
            ...
            if (verifySizes(minRect)) {
                ...
                if (angle - this.angle < 0 && angle + this.angle > 0) {
                    ...
                    warpAffine(src, img_rotated, rotmat, src.size()); // CV_INTER_CUBIC
                    Mat resultMat = showResultMat(img_rotated, rect_size, minRect.center(), k++);
                    if (plateJudge.plateJudge(resultMat) == 1) {
                        //进行马赛克处理
                        Rect rect = crop(minRect.boundingRect(),0.9f,0.8f);
                        mosaic(result, rect);
                        iRet++;
                    }
                }
        ...
        //如果存在车牌，则重写图片到目标路径
        if (iRet > 0) {
            String prefix = imgPath.substring(0,imgPath.lastIndexOf("."));
            String ext = imgPath.substring(imgPath.lastIndexOf("."));
            String orig  = prefix+"-orig"+ext;
            imwrite(orig,src);
     ...
     * @return
     */
    private static Rect crop(Rect rect,float xPercent ,float yPercent){
        if(xPercent>1 || xPercent<0 || yPercent>1 || yPercent < 0) return rect;
        int x = rect.x();
        int y = rect.y();
        int width = rect.width();
        int height = rect.height();

        int xOffset = Math.round(width*(1-xPercent)/2);
        int yOffset = Math.round(height*(1-yPercent)/2);
        return  new Rect(x+xOffset,y+yOffset,Math.round(width*xPercent),Math.round(height*yPercent));
    }
