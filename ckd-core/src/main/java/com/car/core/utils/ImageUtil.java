...
**/
    ...
    public static void resize(String srcImagePath, String destImagePath, int width, int height,
        ...
        if (w < width || h < height) {
            // 不压缩-是否加水印
            if (needWatermark) {
                addImgWatermark(srcImagePath, destImagePath, 100);// 不压缩，加水印
            }
            return;
        }
         **/
    ...
    public static void resizeWithCrop(String srcImagePath, String destImagePath, int width, int height) throws Exception {
        BufferedImage buffimg = ImageIO.read(new File(srcImagePath));
        int w = buffimg.getWidth();
        int h = buffimg.getHeight();
        IMOperation op = new IMOperation();
        float key = height*1.0f/width;
        if(h*1.0f/w>key){
            op.gravity("center");op.addRawArgs("-crop",w+"x"+Math.round(w*key)+"+0+0");
        }else{
            op.gravity("center");op.addRawArgs("-crop",Math.round(h/key)+"x"+h+"+0+0");
        }
        if(w/width>2.0 || h/height>2.0){
            op.addRawArgs("-sharpen","1x1");
        }
        if(height <=0){
     ...
     * @throws Exception
     */
    public static void resizeWithCrop(String urlPath, OutputStream output, int width, int height) throws Exception {
            BufferedImage buffimg = ImageIO.read(new URL(urlPath));
            ...
            if (h * 1.0f / w > key) {
                op.gravity("center");
                op.addRawArgs("-crop", w + "x" + Math.round(w * key) + "+0+0");
            } else {
                op.gravity("center");
                op.addRawArgs("-crop", Math.round(h / key) + "x" + h + "+0+0");
            }
            if (w / width > 2.0 || h / height > 2.0) {
                op.addRawArgs("-sharpen", "1x1");
            }
...
//    }
    ...
    public static void  main(String args[]){
        ...
        String srcPath = "D:\\picture\\a.jpg";
        String destPath ="D:\\picture\\a_tmb.jpg" ;
        try {
           ImageUtil.resizeWithCrop(srcPath, destPath, 100, 80);
