 ...
 *******************************************************************************/
package com.car.core.web;

import com.car.core.utils.Collections3;
import com.car.core.utils.Encodes;
...
public class Servlets {
	...
	/**
	 * 替换或增加指定url里的参数
	 * @param url 传入的url ex："http://m.andaxauto/car/w86xd.html?source=ax"
	 * @param paramName 参数名
	...
	public static String replaceParam(String url, String paramName, String paramValue) {
		...
		if (index != -1) {
			...
			sb.append(url.substring(0, index)).append(paramName + "=").append(paramValue);
			...
			StringBuilder sb = new StringBuilder();
			if(url.indexOf("?") != -1){
				sb.append(url).append("&"+paramName + "=").append(paramValue);
			}else{
				sb.append(url).append("?"+paramName + "=").append(paramValue);
			}
			url = sb.toString();
